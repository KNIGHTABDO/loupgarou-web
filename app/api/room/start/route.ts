...
export async function POST(req: NextRequest) {
  ...
  const roleMap = assignRoles(players.map((p: { id: string }) => p.id), config);
  await Promise.all(Object.entries(roleMap).map(([pid, role]) => supabase.from("players").update({ role }).eq("id", pid)));
  const alivePlayers = players.map((p: { id: string }) => ({ ...p, role: roleMap[p.id], is_alive: true }));
  const firstRole = getNextNightRole([], alivePlayers);
  await supabase.from("rooms").update({ status: "role_reveal", night_number: 1, current_phase_role: firstRole, completed_night_roles: [] }).eq("id", roomId);
  ...
  fetch(`${process.env.NEXT_PUBLIC_APP_URL ?? ""}/api/room/generate-images`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ roomId }) }).catch(() => {});
